...
设置管脚为输入函数
参数: uint32_t io 要设置的gpio管脚所在的位，比如0x00设置GPIO0 0X02->GPIO1 0X40->GPIO2 0XFF->GPIO[7:0]
			uint32_t io_pull   0:io不配置上拉 1：IO配置上拉
			uint32_t io_invert    0：IO不反相  1:IO反相
举例：GPIO_Set_Input(0x0f);   设置gpio0~gpio3为输出状
			GPIO_Set_Input(BIT0 | BIT1 | BIT2 | BIT3);   设置gpio0~gpio3为输出
*/
void GPIO_Set_Input(uint32_t io_pin,uint32_t io_pull,uint32_t io_invert)
{
	uint32_t i=0,j=0;
	for(i=0x01,j=0;i!=0x10000000;i<<=1,j++)
	{
	...
	PIN_CONFIG->GPIODIR |= io_pin;
	PIN_CONFIG->GPIO_PULL_UP |= io_pull;    								//上拉
	if(!io_invert)  
	PIN_CONFIG->GPIO_POL  &= (~io_pin) ;     							//输入信号不取反
	PIN_CONFIG->GPIO_POL  |= io_invert ;   
}
/*
设置管脚输出高电平函数
参数: uint32_t io 要设置的gpio管脚所在的位，比如0x00设置GPIO0 0X02->GPIO1 0X40->GPIO2 0XFF->GPIO[7:0]
